...
class Expenses extends Component
{
    ...
    // Drill-down desde reportes (ej. estadístico DRACO): acotan el listado
    // a un usuario/sede específicos. Solo llegan por URL, no tienen input propio.
    ...
    public function render()
    {
        ...
        // Drill-down desde reportes
        $scopeLabels = [];
        if ($this->user_id) {
            $q->where('user_id', $this->user_id);
            $scopeLabels[] = 'Usuario: '.(DB::table('users')->where('id', $this->user_id)->value('username') ?? '#'.$this->user_id);
        }
        if ($this->headquarter_id && Schema::hasColumn('expenses', 'headquarter_id')) {
            $q->where('headquarter_id', $this->headquarter_id);
            $scopeLabels[] = 'Sede: '.(DB::table('headquarters')->where('id', $this->headquarter_id)->value('name') ?? '#'.$this->headquarter_id);
        }
    ...
    {
        $route = route('exports.expenses', array_filter([
            'search' => $this->search,
